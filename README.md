...
Этот документ содержит инструкции по развертыванию и использованию on-premise платформы для взаимодействия с документами и демонстрации возможностей ИИ. Платформа объединяет инструменты для обработки документов, векторного поиска, автоматизации рабочих процессов, разработки с AI-ассистентом и пользовательские интерфейсы.
...
- **OpenCode**: Контейнеризированная среда разработки с AI-ассистентом.
...
- Дисковое пространство: Минимум 50 ГБ (Рекомендуется 100 ГБ+)
...
OpenCode — это контейнеризированная среда разработки с предустановленным AI-ассистентом [`opencode`](services/opencode/Dockerfile:80), доступная через SSH.

### Особенности

- **SSH-доступ**: Подключение к контейнеру через SSH (порт 22 внутри контейнера)
- **Предустановленные инструменты**: Neovim, Git, Tmux, Lazygit, Starship, Zoxide, UV, Mise и многие другие
- **Языки программирования**: Node.js, Go, Python, Rust (установлены через Mise)
- **AI-ассистент**: OpenCode CLI для работы с LLM
- **Рабочая директория**: `/home/ubuntu/workspace` (монтируется из `./workspace`)

### Подключение

1. **Настройка пароля** (опционально):

   По умолчанию пароль пользователя `ubuntu` — `123456`. Для изменения установите переменную в `.env`:
...
3. **Подключение по SSH**:
   ...
   Сначала определите порт, на который проброшен SSH контейнера:

   ```bash
   docker port opencode 22
   # Вывод: 0.0.0.0:xxxxx
   ```
   ...
   ```bash
   ssh -p xxxxx ubuntu@localhost
   # Пароль: 123456 (или ваш из USER_PASSWORD)
   ```

   **Или** используйте exec для прямого входа:
...
Конфигурация OpenCode хранится в [`./config/opencode/`](services/opencode/config/opencode/) и монтируется в `/home/ubuntu/.config/opencode`:

- [`opencode.json`](services/opencode/config/opencode/opencode.json) — основная конфигурация с провайдерами LLM
- [`skill/`](services/opencode/config/opencode/skill/) — пользовательские навыки (skills)

По умолчанию настроен провайдер OpenRouter с моделью `openai/gpt-5.1-codex-max`.
...
opencode "объясни этот код" < file.py
```

### Рабочее пространство

Все файлы в директории [`./workspace/`](workspace/) хоста доступны внутри контейнера по пути `/home/ubuntu/workspace/`. Это позволяет редактировать код на хосте и выполнять его в изолированной среде.

### Конфигурация Neovim

Конфигурация Neovim (LazyVim) монтируется из [`./config/nvim/`](config/nvim/) в `/home/ubuntu/.config/nvim`. Вы можете кастомизировать редактор, изменяя файлы в этой директории.
